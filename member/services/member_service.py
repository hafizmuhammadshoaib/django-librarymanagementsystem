...
class MemberService:
    ...
    def set_book_service(self, book_service):
        """Manual dependency injection for cross-app service"""
        self.book_service = book_service

    def get_member_borrowing_stats(self, member_id):
        """Get comprehensive borrowing statistics for a member"""
        total_borrowings = self.borrowing_repository.get_borrowing_count_by_member(member_id)
        active_borrowings = self.borrowing_repository.get_active_borrowings_by_member(member_id)
    ...
    def get_member_active_books(self, member_id):
        ...
        for borrowing in active_borrowings:
            book_data = {
                ...
                "book_title": borrowing.book.title,
                "borrowing_date": borrowing.borrowing_date,
                "days_borrowed": (borrowing.borrowing_date - borrowing.borrowing_date).days  # Placeholder
            }
